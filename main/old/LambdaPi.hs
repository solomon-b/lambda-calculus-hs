...
newtype InferM a = InferM {unInferM :: ExceptT TypeErr (Reader Gamma) a}
...
subst x s = \case
  ...
  Abs y ty t1
    | x /= y && y `notElem` freevars s -> Abs y (subst x s ty) (subst x s t1)
    | otherwise -> error "oops name collision"
  Pi y t1 t2
    | x /= y && y `notElem` freevars s -> Pi y (subst x s t1) (subst x s t2)
    | otherwise -> error "oops name collision"
