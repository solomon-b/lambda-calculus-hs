...
-- | System F with a tactical kernel and unified application syntax.
--
-- System F is the polymorphic lambda calculus. Terms abstract over types with
-- big lambda (@Λα. e@), and types contain universally quantified type variables
-- (@∀α. T@).
--
-- == What Changes
--
-- Two structural changes, one at the surface and one internal.
--
-- At the surface, there is a single application constructor 'Ap' carrying an
-- 'Arg' that is either a 'TmArg' or a 'TpArg'. @f x@ and @f [T]@ parse into the
-- same 'Ap' node with different payloads. Type application is not a separate
-- constructor.
--
-- Internally, the dispatch between term application and type application lives
-- in a user-space tactic ('applyTactic') rather than in the typechecker's
-- top-level pattern match. 'applyTactic' inspects the synthesized type of the
-- head and picks between 'lamElim' and 'forallElim'. Three new kernel
-- primitives ('matchGoal', 'examine', 'die') are what makes this expressible
-- outside the kernel.
--
-- == What This Buys
--
-- For the programmer, one application form instead of two. The term-versus-type
-- distinction moves from the constructor to the argument.
--
-- For the elaborator, extensibility. Adding features where one surface form
-- fans out to multiple typing rules (implicit arguments, subtyping coercions,
-- overloaded operators) becomes a matter of writing new user-space dispatch
-- tactics against the same three kernel primitives. The kernel stays fixed. The
-- dispatch logic that depends on language design choices lives outside it.
--
-- == Key Primitive: examine
--
-- 'examine' runs a synth tactic once, caches the resulting @(SType, Syntax)@
-- pair into a no-op synth, and hands both the cached tactic and the synthesized
-- type to a continuation. The continuation dispatches on the type and feeds the
-- cached tactic to whichever kernel rule fires. The rule runs the cached tactic
-- to get the result without re-typechecking the head. Without this caching, a
-- nested application spine would pay exponential re-typechecking cost.
module Main where
...
main = do
  ...
  test
    ...
    ( Ap
        ( Ap
            (Anno (Forall "a" (TVar "a" `FuncTy` TVar "a")) (TyLam "a" (Lam "x" (Var "x"))))
            (TpArg (Forall "b" (TVar "b" `FuncTy` TVar "b")))
        )
  ...
  testErr
    ...
    (Ap (Anno (BoolTy `FuncTy` BoolTy) (Lam "x" (Var "x"))) (TpArg BoolTy))
  testErr
    "term applied to forall (missing type instantiation)"
    ( Ap
        (Anno (Forall "a" (TVar "a" `FuncTy` TVar "a")) (TyLam "a" (Lam "x" (Var "x"))))
        (TmArg (Anno BoolTy Tru))
    )
  testErr
    "application of non-function non-forall"
    (Ap (Anno BoolTy Tru) (TmArg (Anno BoolTy Tru)))
  testErr
