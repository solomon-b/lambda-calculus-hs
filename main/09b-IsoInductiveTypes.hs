...
-- synthesized. The 'subTactic' rule bridges the two directions.
--
-- Each rule returns the elaborated core IR: 'Check' returns @Type ->
...
--
-- The key additions: 'adtIntro' synthesizes a constructor application
-- by looking up the data type spec, computing the mu-type, and elaborating to
-- 'SFold' with sum/product injections. 'adtElim' checks a pattern match
...
synth = \case
  ...
  Cnstr name tms -> adtIntro name (fmap check tms)
...
check (CaseRec scrut cases) = adtElim (synth scrut) (fmap (\(x, y, z) -> (x, check (foldr Lam z y))) cases)
...
-- is unknown), the let rule evaluates @e@ and stores the resulting value in
...
-- means the rule is able to know the type and can check the arguments then
...
-- ──────────────────────────────────────────────────────── Cnstr⇒
--                    Γ ⊢ Cᵢ args ⇒ μα.T
adtIntro :: Name -> [Check] -> Synth
adtIntro nm checks = Synth $
...
-- | Case-Rec Elimination
--
-- A typical Unfold rule would look like:
--
--   Γ ⊢ e ⇒ μα.T
--  ──────────────────── Unfold
--  Γ ⊢ unfold e ⇒ T[μα.T / α]
...
-- of our Case-Rec we do the unfold within the Case-Rec rule:
--
...
adtElim (Synth scrutinee) cases = Check $ \ty -> do
...
-- rule (used in 'check') synthesizes a type for a term and then verifies
...
-- dedicated rules for records (width and depth) and functions
